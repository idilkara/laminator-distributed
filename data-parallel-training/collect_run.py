#!/usr/bin/env python3
"""Automate attested training runs and capture coordinator logs + verifier output."""
...
def _run(cmd, cwd, capture=True):
    """Run a shell command, optionally capturing stdout for later use."""
    print(f"[collect-run] running: {' '.join(cmd)}", flush=True)
    if capture:
...
def _collect_single_run(project_dir: Path, log_path: Path, run_number: int):
    ...
    try:
        ...
        try:
            ...
            print("[collect-run] warning: docker compose down failed", file=sys.stderr)

    verifier_output = _run(["python", "verifier.py"], project_dir / "data")

    header = f"===== RUN {run_number} ({timestamp}) =====\n"
    verify_header = f"===== VERIFICATION {run_number} ({timestamp}) =====\n"
    ...
    with open(log_path, "a", encoding="utf-8") as fh:
        ...
        if coordinator_logs and not coordinator_logs.endswith("\n"):
            fh.write("\n")
        fh.write("\n")
        fh.write(verify_header)
        fh.write(verifier_output)
        if not verifier_output.endswith("\n"):
            fh.write("\n")
        fh.write("\n")

    print(f"[collect-run] wrote results for run {run_number} to {log_path}", flush=True)


def main():
    parser = argparse.ArgumentParser(description="Collect coordinator logs and verifier output")
    parser.add_argument(
        "--project-dir",
        default=Path(__file__).resolve().parent,
        type=Path,
        help="Path containing docker-compose.yaml (default: script directory)",
    )
    parser.add_argument(
        "--output",
        default="run_log.txt",
